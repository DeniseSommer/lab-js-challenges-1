...
function multiplyBy(numArray, multiplier) {
  ...
  for (let i = 0; i < numArray.length; i++) {
    let iteratedNumber = numArray[i];
    multipliedArray.push(iteratedNumber * multiplier);
  }
...
function uniquifyArray(duplicateWords) {
  ...
  for (let i = 0; i < duplicateWords.length; i++) {
    if (nonDuplicatedArr.indexOf(duplicateWords[i]) === -1) {
      nonDuplicatedArr.push(duplicateWords[i]);
    }
  }
